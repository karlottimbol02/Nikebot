...
(async () => { 
	...
	await page.evaluate(async () =>
		document.querySelectorAll(".checkoutLoginSubmit > input")[0].click(),
	);	
	...
	await page.waitForSelector('.credit-card-iframe-cvv');
	await page.waitFor(500);	

	await page.reload({ waitUntil: ["networkidle0", "domcontentloaded"] });

	await page.evaluate(() =>
		document.querySelectorAll(".credit-card-iframe-cvv")[0].scrollIntoView()
	);
	...
	*/
	...
	if(buy == true){
		await page.evaluate(() =>
		document.querySelectorAll(".ncss-btn-accent")[0].click()
	);		
		//#### LOG / DEBUG
		if(debug == true){	
